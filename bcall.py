...
def mod_bcall(sysargv):
	...
	parser = argparse.ArgumentParser(prog="methylCtools bcall", version="0.9.4", description="calls single-basepair methylation ratios from aligned BS sequencing data", formatter_class=RawTextHelpFormatter)
	...
	def dometrics(m):															# m==0: unconverted, m==1: converted
	...
	for c in reg_ref_list:
		...
		if args.me:																# empty structures for metrics
			...
			metqual = [dict(zip(["C", "CG", "CH", "CHG", "CHH"], [[[0, 0] for x in range(42)], [[0, 0] for x in range(42)], [[0, 0] for x in range(42)], [[0, 0] for x in range(42)], [[0, 0] for x in range(42)]])),
				   dict(zip(["C", "CG", "CH", "CHG", "CHH"], [[[0, 0] for x in range(42)], [[0, 0] for x in range(42)], [[0, 0] for x in range(42)], [[0, 0] for x in range(42)], [[0, 0] for x in range(42)]]))]
					...
					if p in dp:
						...
						if args.to:
							...
							for pr in pileupcolumn.pileups:
								...
								if pr.alignment.qname in once: twice += [pr.alignment.qname]
								else: once += [pr.alignment.qname]
						
						### check context
						cx = dp[p][1]													# context
						if not args.mmr: cx = "C"										# if no mmr caluclation test only C
						...
						if dp[p][0] == "+":												# position strand
							...
							for pr in pileupcolumn.pileups:
								...
								if args.to:												# check for overlapping PE reads
									if pr.alignment.qname in twice and pr.alignment.is_read2: continue
								if args.se:												# skip bases close to end of read
									if pr.qpos < args.se or pr.alignment.rlen-pr.qpos <= args.se: continue									
